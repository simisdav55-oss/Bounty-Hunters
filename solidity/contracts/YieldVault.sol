...
contract YieldVault is Ownable {
    ...
    // FIXED: Cap at periodFinish to prevent phantom rewards
    function rewardPerToken() public view returns (uint256) {
        if (totalSupply == 0) return rewardPerTokenStored;
        uint256 timeSinceLastUpdate = lastUpdateTime;
        if (block.timestamp < periodFinish) {
            timeSinceLastUpdate = block.timestamp - lastUpdateTime;
        } else {
            timeSinceLastUpdate = periodFinish - lastUpdateTime;
        }
        // If period already ended, no additional rewards
        if (block.timestamp > periodFinish) {
